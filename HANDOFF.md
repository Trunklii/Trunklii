...
### 社内レビュー（2026-08-17）からの後日対応

- [ ] **nr. は「七五三」を漢字で統一**（et. は Q&A のみ漢字・他は「753」。nr. 完成時に一括で漢字へ）
- [ ] **Gallery の写真をオリジナルデータに差し替え**（現在は圧縮版）
- [ ] **低解像度PCでの改行制御の方針決定**（現状は `word-break:auto-phrase` のみ）
- [ ] Kimono セクションを写真2列に縮小し、Kimono 詳細ページへ導線を作る
- [ ] 祝日・大安の着付け料金の区分をカレンダーに反映（方針確認待ち）
