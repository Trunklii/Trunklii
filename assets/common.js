...
function buildSearchIndex(){
  ...
  if(d.cancellation){
    push(d.cancellation.heading, d.cancellation.body, BASE+'cancellation-policy/', 'Cancel Policy');
    (d.cancellation.policies||[]).forEach(p=>push(p.period, `${p.fee} ${p.desc}`, BASE+'cancellation-policy/', 'Cancel Policy'));
  }
  if(d.goods){
  ...
  if(d.qa){
    ...
    (d.qa.categories||[]).forEach(cat=>{
      cat.items.forEach(item=>push(item.q, item.a, BASE+'qa/', 'Q&A'));
    });
  }
...
window.TRUNKLII_QA = {
  ...
  showSuggestions(){
    ...
    const items = [];
    (d.qa?.categories||[]).forEach(cat=>cat.items.forEach(i=>items.push(i)));
